...
import { Maximize2, Minimize2, X, ChevronDown, ChevronUp, ExternalLink } from 'lucide-react';
...
interface ProjectCardProps {
  ...
  downloadLink?: string | { label: string; url: string; type?: string; images?: string[] }[];
  index?: number;
}
...
const ProjectCard: React.FC<ProjectCardProps> = ({
  ...
  const handleGalleryClick = (link: { type?: string; images?: string[] }) => {
    if (link.type === 'gallery' && link.images && link.images.length > 0) {
      setShowGallery(true);
  ...
  const nextImage = () => {
    if (Array.isArray(downloadLink)) {
      const galleryLink = downloadLink.find(link => link.type === 'gallery');
      if (galleryLink?.images) {
        setCurrentImageIndex((prev) => (prev + 1) % galleryLink.images!.length);
      }
    }
  };
  ...
  const prevImage = () => {
    if (Array.isArray(downloadLink)) {
      const galleryLink = downloadLink.find(link => link.type === 'gallery');
      if (galleryLink?.images) {
        setCurrentImageIndex((prev) => (prev - 1 + galleryLink.images!.length) % galleryLink.images!.length);
      }
    }
  };
  ...
  return (
    <>
                      ...
                      {Array.isArray(downloadLink) ? (
                        downloadLink.map((link, index) => (
                          link.type === "image" ? (
                            ...
                            <motion.button
                              key={index}
                              onClick={() => handleGalleryClick(link)}
      ...
      {/* Gallery Modal */}
      {showGallery && Array.isArray(downloadLink) && (
        <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50">
          <div className="relative max-w-4xl w-full">
            ...
            <div className="relative">
              {downloadLink.find(link => link.type === 'gallery')?.images?.map((imgSrc, index) => (
                <img
                  key={index}
                  src={imgSrc}
                  alt={`Capture ${index + 1}`}
              ...
              {downloadLink.find(link => link.type === 'gallery')?.images?.length! > 1 && (
                <>
                  <button
                    onClick={prevImage}
